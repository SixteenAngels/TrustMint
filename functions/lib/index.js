...
exports.onFirestoreWrite = exports.getNeonDbBackupStats = exports.syncCollectionToNeonDb = exports.getNeonDbSyncStatus = exports.syncBatchToNeonDb = exports.syncToNeonDb = exports.getBusinessMetrics = exports.trackAnalyticsEvent = exports.processKYC = exports.fetchMarketData = exports.sendSMS = exports.sendEmail = exports.sendPushNotification = exports.airteltigoMoneyPayment = exports.vodafoneCashPayment = exports.mtnMobileMoneyPayment = exports.stripeCreatePaymentIntent = exports.flutterwaveInitiatePayment = exports.executeTrade = exports.getActivityLogs = exports.suspendUser = exports.flagUser = exports.createAnnouncement = exports.moderateContent = exports.flagTrade = exports.getTrades = exports.getAnalytics = exports.updateSupportTicket = exports.createSupportTicket = exports.getUsersByRole = exports.assignUserRole = void 0;
...
exports.assignUserRole = functions.https.onCall(async (data, context) => {
    ...
    if ((adminData === null || adminData === void 0 ? void 0 : adminData.role) !== 'admin' && !(adminData === null || adminData === void 0 ? void 0 : adminData.isAdmin)) {
 ...
 */
exports.getUsersByRole = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const userDoc = await admin.firestore().doc(`users/${context.auth.uid}`).get();
    const userData = userDoc.data();
    const isAdmin = (userData === null || userData === void 0 ? void 0 : userData.role) === 'admin' || (userData === null || userData === void 0 ? void 0 : userData.isAdmin) === true;
    const isManager = (userData === null || userData === void 0 ? void 0 : userData.role) === 'manager' || (userData === null || userData === void 0 ? void 0 : userData.isManager) === true;
    ...
    const users = snapshot.docs.map(doc => {
        ...
        if (isManager && !isAdmin) {
            ...
            const { balance } = userData, safeData = __rest(userData, ["balance"]);
            return Object.assign({ id: doc.id }, safeData);
        }
 ...
 */
exports.updateSupportTicket = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    ...
    await ticketRef.update({
        status,
        notes: notes || ((_a = ticketDoc.data()) === null || _a === void 0 ? void 0 : _a.notes),
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
 ...
 */
exports.getTrades = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const userDoc = await admin.firestore().doc(`users/${context.auth.uid}`).get();
    ...
    const trades = snapshot.docs.map(doc => {
        ...
        if (isManager && !isAdmin) {
            const { total, fees } = tradeData, safeData = __rest(tradeData, ["total", "fees"]);
            return Object.assign({ id: doc.id }, safeData);
        }
        return Object.assign({ id: doc.id }, tradeData);
    });
    return { trades };
});
/**
 ...
 */
exports.flagTrade = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const userDoc = await admin.firestore().doc(`users/${context.auth.uid}`).get();
 ...
 */
exports.flagUser = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const userDoc = await admin.firestore().doc(`users/${context.auth.uid}`).get();
...
exports.suspendUser = functions.https.onCall(async (data, context) => {
    ...
    const userDoc = await admin.firestore().doc(`users/${context.auth.uid}`).get();
    const userData = userDoc.data();
    if ((userData === null || userData === void 0 ? void 0 : userData.role) !== 'admin' && !(userData === null || userData === void 0 ? void 0 : userData.isAdmin)) {
        throw new functions.https.HttpsError('permission-denied', 'Admin privileges required');
    }
...
exports.getActivityLogs = functions.https.onCall(async (data, context) => {
    ...
    let query = admin.firestore().collection('activityLogs');
    if (type) {
...
exports.executeTrade = functions.https.onCall(async (data, context) => {
    ...
    const currentPrice = (stockData === null || stockData === void 0 ? void 0 : stockData.price) || price || 1.0;
 ...
 */
exports.flutterwaveInitiatePayment = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { amount, currency } = data;
    const FLUTTERWAVE_SECRET_KEY = (_a = functions.config().flutterwave) === null || _a === void 0 ? void 0 : _a.secret_key;
    if (!amount || !currency) {
 ...
 */
exports.mtnMobileMoneyPayment = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { amount, phone } = data;
    const MTN_MOMO_API_KEY = (_a = functions.config().mtn) === null || _a === void 0 ? void 0 : _a.api_key;
    if (!amount || !phone) {
 ...
 */
exports.vodafoneCashPayment = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { amount, phone } = data;
    const VODAFONE_API_KEY = (_a = functions.config().vodafone) === null || _a === void 0 ? void 0 : _a.api_key;
    if (!amount || !phone) {
        throw new functions.https.HttpsError('invalid-argument', 'Amount and phone are required');
    }
    // Mock response if no API key
 ...
 */
exports.airteltigoMoneyPayment = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { amount, phone } = data;
    const AIRTELTIGO_API_KEY = (_a = functions.config().airteltigo) === null || _a === void 0 ? void 0 : _a.api_key;
    if (!amount || !phone) {
        throw new functions.https.HttpsError('invalid-argument', 'Amount and phone are required');
    }
    // Mock response if no API key
...
exports.sendPushNotification = functions.https.onCall(async (data, context) => {
    ...
    const FCM_SERVER_KEY = (_a = functions.config().fcm) === null || _a === void 0 ? void 0 : _a.server_key;
...
exports.sendEmail = functions.https.onCall(async (data, context) => {
    ...
    const SENDGRID_API_KEY = (_a = functions.config().sendgrid) === null || _a === void 0 ? void 0 : _a.api_key;
...
exports.sendSMS = functions.https.onCall(async (data, context) => {
    ...
    const TWILIO_ACCOUNT_SID = (_a = functions.config().twilio) === null || _a === void 0 ? void 0 : _a.account_sid;
    const TWILIO_AUTH_TOKEN = (_b = functions.config().twilio) === null || _b === void 0 ? void 0 : _b.auth_token;
    if (!to || !message) {
 ...
 */
exports.fetchMarketData = functions.https.onCall(async (data, context) => {
    var _a, _b;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const { symbol } = data;
    const TWELVE_DATA_API_KEY = (_a = functions.config().twelvedata) === null || _a === void 0 ? void 0 : _a.api_key;
    const ALPHA_VANTAGE_API_KEY = (_b = functions.config().alphavantage) === null || _b === void 0 ? void 0 : _b.api_key;
    if (!symbol) {
...
exports.processKYC = functions.https.onCall(async (data, context) => {
    ...
    const SMILE_ID_API_KEY = (_a = functions.config().smileid) === null || _a === void 0 ? void 0 : _a.api_key;
    const JUMIO_API_KEY = (_b = functions.config().jumio) === null || _b === void 0 ? void 0 : _b.api_key;
...
exports.trackAnalyticsEvent = functions.https.onCall(async (data, context) => {
    ...
    const GOOGLE_ANALYTICS_ID = (_a = functions.config().analytics) === null || _a === void 0 ? void 0 : _a.ga_id;
    const MIXPANEL_TOKEN = (_b = functions.config().analytics) === null || _b === void 0 ? void 0 : _b.mixpanel_token;
    if (!eventName) {
 ...
 */
exports.getBusinessMetrics = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const userDoc = await admin.firestore().doc(`users/${context.auth.uid}`).get();
 ...
 */
exports.syncToNeonDb = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    ...
    const NEON_DB_URL = process.env.NEON_DB_URL ||
        ((_a = functions.config().neondb) === null || _a === void 0 ? void 0 : _a.url) ||
    ...
    `);
       ...
       ON CONFLICT (id) 
       DO UPDATE SET data = $2, updated_at = CURRENT_TIMESTAMP`, [documentId, JSON.stringify(documentData)]);
        await pool.end();
        // Log sync
 ...
 */
exports.syncBatchToNeonDb = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    ...
    const NEON_DB_URL = ((_a = functions.config().neondb) === null || _a === void 0 ? void 0 : _a.url) || process.env.NEON_DB_URL;
    if (!NEON_DB_URL) {
        return {
    ...
    `);
        ...
        for (const doc of documents) {
           ...
           VALUES ($1, $2, CURRENT_TIMESTAMP)
           ON CONFLICT (id) 
           DO UPDATE SET data = $2, updated_at = CURRENT_TIMESTAMP`, [doc.id, JSON.stringify(doc.data)]);
                results.push({
                    collection,
                    documentId: doc.id,
                    synced: true,
 ...
 */
exports.getNeonDbSyncStatus = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    ...
    try {
        ...
        const tableName = `firestore_${collection}`;
        const result = await pool.query(`SELECT id, synced_at, updated_at FROM ${tableName} WHERE id = $1`, [documentId]);
        await pool.end();
 ...
 */
exports.syncCollectionToNeonDb = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
 ...
 */
exports.getNeonDbBackupStats = functions.https.onCall(async (data, context) => {
    var _a, _b;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    ...
    `);
        const collections = tablesResult.rows.map((row) => row.table_name);
        ...
        return {
            ...
            lastSyncTime: lastSyncLog.empty
                ? undefined
                : (_b = lastSyncLog.docs[0].data().syncedAt) === null || _b === void 0 ? void 0 : _b.toDate().toISOString(),
            syncEnabled: true,
        };
    }
    catch (error) {
